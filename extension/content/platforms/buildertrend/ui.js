...
const SurfaceFlowUI = {
  ...
  init(context, jobData) {
    ...
    // Check if we should inject the button
    ...
    const hasJobInfo = document.querySelector('[data-testid="jobInfo.jobName"]') !== null;
    
    console.log('[SurfaceFlow] Detection checks:', { hasJobModal, hasJobHeader, hasJobTabs, hasJobInfo });
    
    // Inject on job pages, modals, or any page with job data
    if (context.pageType === 'job_page' || context.pageType === 'job_detail' || 
        url.includes('/JobPage/') || url.includes('/Job/') ||
        hasJobModal || hasJobHeader || hasJobTabs || hasJobInfo) {
  ...
  injectHotelBookingButton() {
    ...
    // Strategy 1: Find jobTabsButtonGroup (modal tabs)
    const jobTabsButtonGroup = document.querySelector('[data-testid="jobTabsButtonGroup"]');
    if (jobTabsButtonGroup) {
      console.log('[SurfaceFlow] Found jobTabsButtonGroup in modal, inserting button');
      const container = this.createButtonElement();
      jobTabsButtonGroup.appendChild(container);
      this.attachButtonHandler();
      return;
    }

    // Strategy 2: Find jobTabsActionBar 
    const jobTabsActionBar = document.querySelector('[data-testid="jobTabsActionBar"]');
    if (jobTabsActionBar) {
      console.log('[SurfaceFlow] Found jobTabsActionBar, inserting button');
      const container = this.createButtonElement();
      jobTabsActionBar.appendChild(container);
      this.attachButtonHandler();
      return;
    }

    // Strategy 3: Find modal footer action bar
    const modalFooter = document.querySelector('[data-testid="jobPresentationalActionBar"]');
    if (modalFooter) {
      console.log('[SurfaceFlow] Found modal action bar, inserting button');
      const container = this.createButtonElement();
    ...
    // Strategy 4: Find the last tab element and insert button after it (inline with tabs)
    ...
    // Strategy 5: Find tablist and append inside it
    ...
    // Strategy 6: Look for other common containers
  ...
  openHotelSearchModal() {
    ...
    // ========== IMMEDIATELY SEND DATA TO BACKEND ==========
    console.log('[SurfaceFlow] 🚀 Immediately sending data to backend on button click!');
  ...
  async startSearchAnimation() {
    ...
    // Backend API was already called immediately on button click
  ...
  async handleBookingRequest() {
    ...
    if (this.backendBookingJobId) {
      try {
        ...
        const response = await fetch('http://127.0.0.1:8000/api/v1/buildertrend/hotel-booking/approve/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Extension-Id': 'surfaceflow-ai'
